...
<!-- ============================================================ -->
<!-- STEP 2: WAF RULE DEPLOYMENT                                  -->
<!-- ============================================================ -->
<h3>Step 2: WAF Rule Deployment</h3>
<p>Deploy the block rule <code>ip.src in $<?php echo htmlspecialchars( $fb_list_id, ENT_QUOTES, 'UTF-8' ); ?></code> to each zone. It is inserted as the <strong>first (highest-priority) rule</strong> in the zone&rsquo;s WAF ruleset&mdash;all existing rules are preserved. Click <em>Check WAF Status</em> to load zone-level status for an account.</p>

<?php foreach ( $fb_account_ids as $idx => $account_id ) : ?>
	<?php
	$safe_id       = htmlspecialchars( $account_id, ENT_QUOTES, 'UTF-8' );
	$waf_disp_name = isset( $fb_account_names[ $idx ] ) ? $fb_account_names[ $idx ] : ( 'Account ' . ( $idx + 1 ) );
	?>
<div class="fb-account-block">
	<div class="fb-account-block-header">
		<strong><?php echo htmlspecialchars( $waf_disp_name, ENT_QUOTES, 'UTF-8' ); ?></strong>
		<small style="font-family:monospace;color:#666"><?php echo $safe_id; ?></small>
		<button class="fb-btn outline fb-check-waf-btn"
			data-account-id="<?php echo $safe_id; ?>"
			data-idx="<?php echo $idx; ?>">
			Check WAF Status
		</button>
		<button class="fb-btn fb-deploy-all-btn"
			data-account-id="<?php echo $safe_id; ?>"
			data-idx="<?php echo $idx; ?>"
			style="display:none">
			Deploy to All Zones
		</button>
		<span class="fb-msg" id="fb-waf-msg-<?php echo $idx; ?>"></span>
	</div>
	<div id="fb-zones-<?php echo $idx; ?>" class="fb-zones-container"></div>
</div>
<?php endforeach; ?>
...
<?php endforeach; ?>
	...
	// ---------------------------------------------------------------
	// STEP 2: Check WAF Status per account.
	// ---------------------------------------------------------------
	document.querySelectorAll('.fb-check-waf-btn').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var idx       = btn.dataset.idx;
			var accountId = btn.dataset.accountId;
			var zonesEl   = document.getElementById('fb-zones-' + idx);
			var msgEl     = document.getElementById('fb-waf-msg-' + idx);
			var deployBtn = btn.parentElement.querySelector('.fb-deploy-all-btn');

			btn.disabled = true;
			setMsg(msgEl, 'Loading zones\u2026', '');
			if (zonesEl) zonesEl.innerHTML = '';

			ajax({ setting: 'fail2ban_check_waf_status', account_id: accountId })
				.then(function (data) {
					btn.disabled = false;
					if (!data.success) {
						setMsg(msgEl, 'Error: ' + (data.message || 'Unknown'), 'err');
					...
					if (zones.length === 0) {
						setMsg(msgEl, 'No zones found in this account.', '');
						return;
					}

					setMsg(msgEl, zones.length + ' zone(s) loaded.', 'ok');
					if (deployBtn) deployBtn.style.display = 'inline-block';

					zonesEl.innerHTML = zones.map(function (z) {
						var badgeCls  = z.has_rule ? 'on' : 'off';
						var badgeTxt  = z.has_rule ? '&#10003; Rule active' : '&#10007; Not deployed';
						var deployBtn = z.has_rule
							? '<button class="fb-btn outline fb-zone-deploy-btn" data-zone-id="' + escHtml(z.zone_id) + '" data-idx="' + escHtml(String(idx)) + '" disabled style="opacity:.5">Deployed</button>'
							: '<button class="fb-btn fb-zone-deploy-btn" data-zone-id="' + escHtml(z.zone_id) + '" data-idx="' + escHtml(String(idx)) + '">Deploy Rule</button>';
						return '<div class="fb-zone-row" id="fb-zone-' + escHtml(z.zone_id) + '">'
							+ '<span class="fb-zone-name">' + escHtml(z.zone_name) + '</span>'
							+ '<span class="fb-badge ' + badgeCls + '">' + badgeTxt + '</span>'
							+ deployBtn
							+ '<span class="fb-msg" id="fb-zone-msg-' + escHtml(z.zone_id) + '"></span>'
							+ '</div>';
					}).join('');

					// Attach deploy handlers to the freshly rendered buttons.
					attachZoneDeployHandlers(zonesEl);
				})
				.catch(function (e) {
					btn.disabled = false;
					setMsg(msgEl, 'Network error: ' + e.message, 'err');
	...
	// ---------------------------------------------------------------
	// Deploy WAF rule to a single zone.
	// ---------------------------------------------------------------
	function attachZoneDeployHandlers(container) {
		container.querySelectorAll('.fb-zone-deploy-btn').forEach(function (btn) {
			btn.addEventListener('click', function () {
				var zoneId = btn.dataset.zoneId;
				var msgEl  = document.getElementById('fb-zone-msg-' + zoneId);

				btn.disabled = true;
				setMsg(msgEl, 'Deploying\u2026', '');

				ajax({ setting: 'fail2ban_create_waf_rule', zone_id: zoneId })
					.then(function (data) {
						if (data.success) {
							var row = document.getElementById('fb-zone-' + zoneId);
							if (row) {
								var badge = row.querySelector('.fb-badge');
								if (badge) { badge.className = 'fb-badge on'; badge.innerHTML = '&#10003; Rule active'; }
							}
							btn.textContent = 'Deployed';
							btn.classList.add('outline');
							setMsg(msgEl, data.message || 'Done.', 'ok');
						} else {
							btn.disabled = false;
							setMsg(msgEl, 'Error: ' + (data.message || 'Unknown'), 'err');
						}
	...
	// ---------------------------------------------------------------
	// Deploy WAF rule to ALL zones in an account.
	// ---------------------------------------------------------------
	document.querySelectorAll('.fb-deploy-all-btn').forEach(function (btn) {
		btn.addEventListener('click', function () {
			var idx     = btn.dataset.idx;
			var msgEl   = document.getElementById('fb-waf-msg-' + idx);
			var zonesEl = document.getElementById('fb-zones-' + idx);
			...
			var zoneBtns = zonesEl.querySelectorAll('.fb-zone-deploy-btn:not(:disabled)');
			if (zoneBtns.length === 0) {
				setMsg(msgEl, 'All zones already have the rule.', 'ok');
				return;
			}

			setMsg(msgEl, 'Deploying to ' + zoneBtns.length + ' zone(s)\u2026', '');
			btn.disabled = true;

			var pending = zoneBtns.length;
			var errors  = 0;

			zoneBtns.forEach(function (zb) {
				// Simulate click on each individual zone deploy button.
				zb.click();
				// Track completion by polling the msg element (simple approach).
				var zoneId = zb.dataset.zoneId;
				var zoneMsgEl = document.getElementById('fb-zone-msg-' + zoneId);
				var timer = setInterval(function () {
					if (zb.disabled) { // button got disabled = request finished
						clearInterval(timer);
						if (zoneMsgEl && zoneMsgEl.classList.contains('err')) errors++;
						pending--;
						if (pending === 0) {
							btn.disabled = false;
							setMsg(msgEl, errors === 0 ? 'All zones deployed.' : errors + ' error(s) \u2014 check zone rows.', errors ? 'err' : 'ok');
						}
					}
				}, 200);
			});
